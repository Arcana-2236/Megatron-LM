...
# tp_mode="auto" select tp_mode per weight (Dense/GTP weights only)
...
def _select_tp_mode(
    ...
    """
    m, n = min(M, N), max(M, N)
    K = M // G  # dist orthogonalizes the [N, M/G] shard; transpose is forced
    ...
    if profile is None:
        return "duplicated" if crosses_domain else min(candidates, key=flops.get)

    wire_frac = (G - 1) / G  # ring: each rank moves (G-1)/G of the buffer
    wire = {
        "duplicated": M * N * elem_size * wire_frac,  # one all-gather
        "distributed": steps * 2 * N * N * elem_size * wire_frac,  # gram all-reduce per step
    }
    bw = (profile.bw_inter_gbps if crosses_domain else profile.bw_intra_gbps) * 1e9
    peak = profile.bf16_peak_tflops * 1e12
    cost = {m: flops[m] / peak + wire[m] / bw for m in candidates}
...
class TensorParallelMuon(OrthogonalizedOptimizer):
    ...
    def _resolve_tp_mode(self, M: int, N: int, G: int) -> str:
        """Cached per-shape mode for tp_mode="auto", dense (GTP) weights only.

        crosses_domain=False always: this is only called for dense weights (see
        scaled_orthogonalize_fn_with_gtp_remat), and GTP stays inside one NVLink domain.
        """
        key = (M, N, G)
        if key not in self._tp_mode_cache:
            self._tp_mode_cache[key] = _select_tp_mode(
                ...
                crosses_domain=False, profile=self._hw_profile,
            )
            log_single_rank(
                logger,
                logging.INFO,
                f"muon tp_mode=auto (dense): ({M}, {N}) G={G} -> {self._tp_mode_cache[key]}",
            )
    ...
    def scaled_orthogonalize_fn_with_gtp_remat(self, p, grad, tp_group, partition_dim):
        ...
        if not needs_two_step_communication:
            ...
            return self.scaled_orthogonalize_fn(grad, gtp_remat_group, partition_dim=0, mode=mode)
